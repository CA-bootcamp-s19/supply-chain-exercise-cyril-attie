...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
   ...
   */
  modifier forSale(uint sku) {require(items[sku].price != 0 && items[sku].state == State(0)); _;}
  modifier sold(uint _sku) {require(items[_sku].state == State(1)); _;}
  modifier shipped(uint _sku) {require(items[_sku].state == State(2)); _;}
  modifier received(uint _sku) {require(items[_sku].state == State(3)); _;}
  ...
  {
    items[sku].state = State(uint(items[sku].state)+1);
